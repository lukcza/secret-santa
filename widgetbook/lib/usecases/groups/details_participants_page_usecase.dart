...
final detailsParticipantsPageComponent = WidgetbookComponent(
  ...
  useCases: [
    // ── 1. Regular member (can be removed) ───────────────────────────────────
    WidgetbookUseCase(
      name: 'Regular member',
      builder: (context) {
        ...
        final hasExclusions = context.knobs.boolean(
          label: 'Has pre-existing exclusions',
          initialValue: false,
        );
        ...
        return DetailsParticipantsPage(
          ...
          initialExcludedUIDs:
              hasExclusions
                  ? [_participants[0].uid, _participants[2].uid]
                  : const [],
    ...
    // ── 3. Solo participant (no others → empty exclusions hint) ───────────────
    WidgetbookUseCase(
      ...
      builder: (context) {
        return DetailsParticipantsPage(
          ...
          group: GroupEntity(
            ...
            title: 'Solo Group',
            authorUID: 'admin_uid',
            participants: {'admin_uid': UserStatus.creator},
    ...
    // ── 4. All excluded ───────────────────────────────────────────────────────
    WidgetbookUseCase(
      name: 'All participants excluded',
      builder: (context) {
        return DetailsParticipantsPage(
          ...
          initialExcludedUIDs:
              _participants
                  .map((u) => u.uid)
                  .where((uid) => uid != _participants[1].uid)
                  .toList(),
